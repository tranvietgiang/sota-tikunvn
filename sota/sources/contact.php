<?php
	...
	function get_items()
	{
		...
		$total = $count['num'];
		$url = "index.php?com=contact&act=man&type='".$type."'";
	...
	function get_item()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=contact&act=man&type=".$type."&p=".$curPage, false);

		$item = $d->rawQueryOne("select * from #_contact where id = ? limit 0,1",array($id));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=contact&act=man&type=".$type."&p=".$curPage, false);
	}
	...
	function save_item()
	{
		...
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=contact&act=man&type=".$type."&p=".$curPage, false);
		...
		if($id)
		{
			...
			$d->where('id', $id);
			if($d->update('contact',$data)) $func->redirect("index.php?com=contact&act=man&type=".$type."&p=".$curPage);
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			if($row['id'])
			{
				...
				$func->redirect("index.php?com=contact&act=man&type=".$type."&p=".$curPage);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=contact&act=man&type=".$type."&p=".$curPage, false);
		}
