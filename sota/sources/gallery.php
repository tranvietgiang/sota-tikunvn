	...
	function save_photo()
	{
		...
		if($id)
		{
			if(isset($_FILES['file']))
			{
				...
				if($photo = $func->uploadImage("file", $config[$com][$type][$dfgallery][$val]['img_type_photo'], "../upload/".$com."/",$file_name))
				{
					...
					if($row['id']) $func->delete_file("../upload/".$com."/".$row['photo']);
				}
			}
			...
			$d->where('kind', $kind);
			$d->where('val', $val);
			if($d->update('gallery',$data)) $func->redirect("index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage);
		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				if(isset($flag_fail) && isset($flag_success))
				{
					if(($flag_fail == count($dataMultiTemp)) || $flag_success == 0)
					{
						$func->transfer("Lưu dữ liệu thất bại", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage, false);
					}
					else if($flag_success > 0)
					{
						if($flag_fail > 0)
						{
							$func->transfer("Lưu ".$flag_success." mục dữ liệu thành công. </br> Trong đó có ".$flag_fail." mục bị lỗi", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage);
						}
						else
						{
							$func->redirect("index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage);
						}
					}
				}
				else
