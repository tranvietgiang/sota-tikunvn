		...
		<div class="row">
			<div class="<?= $colLeft ?>">
				<div id="scroll-left">
					...
					<!-- Đánh giá (Đang phát triển) //TODO-->
					<?php /* if (isset($config['product'][$type]['danhgia']) && $config['product'][$type]['danhgia'] == true) { ?>
						<div class="card card-primary card-outline text-sm">
							<div class="card-header">
								<h3 class="card-title">Đánh giá <?= $config['product'][$type]['title_main'] ?></h3>
								<div class="card-tools">
									<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
								</div>
							</div>
									...
									<div class="card-body card-article">
										<div class="tab-content" id="custom-tabs-three-tabContent-lang">
											<?php foreach ($config['website']['lang'] as $k => $v) { ?>
												<div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
													<div class="form-group">
														<label for="ten<?= $k ?>">Người dùng (<?= $k ?>):</label>
														<input type="text" class="form-control for-seo" name="data[ten<?= $k ?>]" id="ten<?= $k ?>" placeholder="Tiêu đề (<?= $k ?>)" value="<?= @$item['ten' . $k] ?>" <?= ($k == 'vi') ? 'required' : '' ?>>
													</div>
													<?php if (isset($config['product'][$type]['motangan']) && $config['product'][$type]['motangan'] == true) { ?>
														<div class="form-group">
				...
				<div id="scroll-right">
					...
					<div class="card card-primary card-outline text-sm">
						...
						<div class="card-body">
							...
							<?php if (isset($config['product'][$type]['danhgia']) && $config['product'][$type]['danhgia'] == true && $act != 'add' && count($getRating) > 0) { ?>
								
								<div class="form-group">
									<label for="exampleFormControlSelect1">Quản Lý đánh giá</label>
									<select class="form-control" id="ma_rank" name="ma_rank">
										<option selected disabled>Chọn quản lý</option>
										<option value="5">Xóa đánh giá dưới 5 sao</option>
										<option value="4">Xóa đánh giá dưới 4 sao</option>
										<option value="3">Xóa đánh giá dưới 3 sao</option>
										<option value="2">Xóa đánh giá dưới 2 sao</option>
									</select>
								</div>
								<?php
								if (count($getRating)) {
									$sumRating = array(); ?>
									<ul class="form-group">
										<li>Đánh giá hiện tại (<?= count($getRating) ?> đánh giá)</li>
										<?php foreach ($getRating as $key => $value) { ?>
											<li style="list-style: none;">
												<?php for ($i = 0; $i < $value['rating']; $i++) {  ?>
													<i class="fas fa-star" style="color:#febb0d;"></i>
												<?php } ?>
											</li>
										<?php
										array_push($sumRating, $value['rating']);
									} ?>
										<li>Đánh giá trung bình: <strong><?=number_format(array_sum($sumRating)/count($getRating), 1, '.', '')?></strong> <i class="fas fa-star" style="color:#febb0d;"></i></li>
									</ul>
								<?php } ?>
							<?php } ?>
							<div class="row">
