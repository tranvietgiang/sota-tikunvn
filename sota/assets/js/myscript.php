...
function setMinDateTime() {
    ...
    // Đảm bảo đồng bộ giây và mili-giây về 0 để tránh lỗi so sánh
    now.setSeconds(0, 0);
...
function validateDateTime() {
    ...
    // Đồng bộ giây và mili-giây để so sánh chuẩn xác
    now.setSeconds(0, 0);

    if (selectedTime <= now) {
        errorMsg.textContent = "⛔ Không thể chọn thời gian nhỏ hơn hoặc bằng hiện tại. Hãy chọn lại!";
        input.value = "";
        input.focus();
        return false;
    } else {
            ...
            onEnd: function() {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        colfiler: colfiler,
                        cmd: 'updateNumb',
                        hash: '<?=$func->generateHash()?>'
                    },
                    success: function(result) {
                        var arrid = result.id;
                        var arrnumb = result.numb;
                        for (var i = 0; i < arrid.length; i++) $('.my-jFiler-item-' + arrid[i])
                            .find("input[type=number]").val(arrnumb[i]);
                    }
                })
            },
        });
    ...
    if (inputFile[0].files[0]) {
        if (inputFile[0].files[0].name.match(/.(jpg|jpeg|png|gif|webp)$/i)) {
            ...
            if (size <= 4096) {
                ...
                notifyDialog("Dung lượng hình ảnh lớn. Dung lượng cho phép <= 8MB ~ 8192KB");
                return false;
            }
        } else {
...
function previewWatermark() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            toDataURL(
                'index.php?com=photo&act=preview-watermark&type=<?=(isset($type) && $type != '') ? $type : ''?>&position=' +
                data.position + '&img=' + data.image + '&watermark=' + data.path + '&upload=' + data
                .upload + '&opacity=' + data.data.opacity + '&per=' + data.data.per + '&small_per=' +
                data.data.small_per + '&min=' + data.data.min + '&max=' + data.data.max + "&t=" + data
                .time,
                function(dataUrl) {
...
$(document).ready(function() {
    ...
    $("#filer-gallery").attr({
        'data-params': '<?=base64_encode($_SERVER['QUERY_STRING'])?>',
        'data-hash': '<?=$func->generateHash()?>'
    });
...
$('.btn_chat_ai').click(function(e) {
    ...
    // 						result = result.replaceAll("*","");
    // 						CKEDITOR.instances.noidungvi.setData(result);
    // 						holdonClose();
    // 					},
    ...
    // 		});
    // 	} else {
    // 		alert("Vui lòng nhập từ khóa vào tiêu đề Tiếng Việt")
    // 		holdonClose();
    // 	}
...
$('.submit_ai').click(function(e) {
    if ($('#tukhoa__ai').val() == '') {
        ...
        setTimeout(function() {
            $.ajax({
                url: 'ajax/ajax_ai_dany.php',
                type: "POST",
                async: true,
                data: {
                    key: $('#tukhoa__ai').val(),
                    pc: $('#phongcach__ai').val(),
                    dd: $('#ddoai__ai').val(),
                },
                success: function(result) {
                    let data = JSON.parse(result);
                    //	console.log(result)
                    data['result'] = data['result'].substr(8, data['result'].length - 8);
                    data['result'] = data['result'].substr(0, data['result'].length - 4);
                    myArr = JSON.parse(data['result']);
                    var html = '';
                    for (let index = 0; index < myArr.length; index++) {
                        html += `
								<div class="items_dany_h2">
									<div class="item_h2"><span>H2</span> <span style="font-weight:600;"><input id="h2_` + index +
                            `" type="text" value="` + myArr[index]['h2'] + `" readonly></span></div>
										<div class="items_dany_h3">`;
                        for (let index1 = 0; index1 < myArr[index]['h3'].length; index1++) {
                            html += `<div class="item_dany_h3">
												<span>H3</span> <span><input id="h3_` + index + `_` + index1 + `" type="text" value="` + myArr[index][
                                'h3'
                            ][index1] + `" readonly></span>
											</div>`;
                        }
                        html += `</div>
									</div>
...
var html_dany = '';
$('.submit__ai').click(function(e) {
    html_dany = '[';
    for (let index = 0; index < $('.items_dany_h2').length; index++) {
        //kiểm tra thẻ h2
        console.log($('#h2_' + index).val() != null, $('#h2_' + index).val() != '')
        if ($('#h2_' + index).val() != null && $('#h2_' + index).val() != '') {
            //gán thẻ h2
            html_dany += '{"h2":"' + $('#h2_' + index).val() + '"';
            //nếu có tồn tại thẻ h3
            if ($('.items_dany_h2').eq(index).find('.item_dany_h3').length > 0) {
                html_dany += ',"h3":[';
            }
            //duyệt qua các thẻ h3
            for (let index1 = 0; index1 < $('.items_dany_h2').eq(index).find('.item_dany_h3')
                .length; index1++) {
                //gán thẻ h3
                html_dany += '"' + $('#h3_' + index + '_' + index1).val() + '"';
                //kiểm tra thẻ h3 cuối cùng
                if ((index1 + 1) < $('.items_dany_h2').eq(index).find('.item_dany_h3').length) {
                    html_dany += ','; //còn thẻ h3 
                } else {
                    html_dany += ']'; //thẻ h3 cuối cùng
                }
            }
            html_dany += '}';
            //kiểm trả thẻ h2 cuối cùng
            if ((index + 1) < $('.items_dany_h2').length) {
                if ($('#h2_' + (index + 1)).val() != null && $('#h2_' + (index + 1)).val() != '') {
                    html_dany += ',';
                }
            }
        } else {
            if ((index + 1) < $('.items_dany_h2').length && html_dany != '[') {
                if ($('#h2_' + (index + 1)).val() != null && $('#h2_' + (index + 1)).val() != '') {
                    html_dany += ',';
                }
            }
    ...
    setTimeout(function() {
        $.ajax({
            ...
            success: function(result) {
                //	$response = preg_replace('/```(?:html)?|```/', '', $result);
                result = result.replaceAll("**", "");
                result = result.replaceAll("*", "");
                $('.noidung__ai').html(result)
...
$('.submit_apdung').click(function(e) {
    $('#slugvi').val(changeTitle($('#tieude__ai').val()))
    $('#tenvi').val($('#tieude__ai').val())
    $('#motavi').val($('.mota__ai').html())
    CKEDITOR.instances.noidungvi.setData($('.noidung__ai').html());
    $('.close_modal').trigger("click")
    holdonClose();
})

$('#check_dany').change(function() {
    if ($(this).is(":checked")) {
        $('.item_h2 input').prop("readonly", false);
        $('.item_dany_h3 input').prop("readonly", false);
        $('.item_h2 input').css({
            border: '1px solid red'
        })
        $('.item_dany_h3 input').css({
            border: '1px solid red'
        })
    } else {
        $('.item_h2 input').prop("readonly", true);
        $('.item_dany_h3 input').prop("readonly", true);
        $('.item_h2 input').css({
            border: 'none'
        })
        $('.item_dany_h3 input').css({
...
$(document).on('change', '.item_h2 input', function() {

})
$(document).on('change', '.item_dany_h3 input', function() {
...
function changeTitle(text = '') {
    if (text !== '') {
        text = utf8Convert(text.toLowerCase());
        text = text.replace(/[^a-z0-9-\s]/g, '');
        text = text.replace(/[\s]+/g, '-');
        text = text.replace(/%20|\s/g, '-');
        text = text.replace(/-{2,}/g, '-'); // gom các dấu - liên tiếp thành 1 dấu -
        text = '@' + text + '@';
        text = text.replace(/@\-|\-\@|@/g, '');
    }
    return text;
